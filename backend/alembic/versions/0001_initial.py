...
def upgrade() -> None:
    ...
    feedback_status = sa.Enum("draft", "submitted", "flagged", "retracted", name="feedback_status")
    ...
    op.create_index("ix_org_subordinates_descendant", "org_subordinates", ["descendant_user_id"])
    ...
    op.create_table(
        ...
        sa.Column("parent_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("topic_taxonomy.id")),
        sa.Column("active", sa.Boolean, nullable=False, server_default=sa.true()),
        sa.Column(
            "merged_into_id",
            postgresql.UUID(as_uuid=True),
    ...
    op.create_table(
        ...
        sa.Column(
            "conversation_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("conversations.id")
        ),
    ...
    op.execute("CREATE INDEX ix_feedback_chunks_tsv ON feedback_chunks USING gin (tsv)")
    op.execute(
